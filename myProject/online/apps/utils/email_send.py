...
def send_link_email(email, send_type='register'):
    ...
    if send_type == 'register':
        email_subject = '宅家学习网激活邮件（请勿回复）'
        email_message = '欢迎您注册宅家学习网账号，请点击下面的链接完成激活:\nhttp://127.0.0.1:8000/users/active/' + emailinfo.code
        send_mail(email_subject, email_message, EMAIL_FROM, [email])
        emailinfo.save()
    elif send_type == 'forget':
        email_subject = '学习网密码重置邮件（请勿回复）'
        email_message = '欢迎您使用宅家学习网平台，请点击下面的链接重置登录密码:\nhttp://127.0.0.1:8000/users/pwdreset/' + emailinfo.code
        send_mail(email_subject, email_message, EMAIL_FROM, [email])
        emailinfo.save()
    elif send_type == 'update_email':
        email_subject = '宅家学习网邮箱重置邮件（请勿回复）'
        email_message = '欢迎您使用宅家学习网平台，邮箱验证码为:' + emailinfo.code
        send_mail(email_subject, email_message, EMAIL_FROM, [email])
        emailinfo.save()
        pass
